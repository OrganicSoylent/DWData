...
server <- function(input, output) { # , session
  ...
  withProgress(message = 'verarbeite Stationslexikon', value = 0, {
    ...
    ## creating SpatialPointsDataframe for map
    pt <- tab2
    ...
    #### map ####
    pt2 <- eventReactive(selected(),
                         pt[which(!is.na(match(pt$ID, tb()[selected(),"ID"]))), ])
    
    output$mapplot <- renderLeaflet({
      leaflet() %>%
        addProviderTiles("OpenTopoMap",providers$Stamen.TonerLite,
                         options = providerTileOptions(noWrap = TRUE)) %>%
        addCircleMarkers(data = pt2(), radius = 2, fillOpacity = .33, opacity = 1, label = pt2()$Station,
                         popup = paste(sep = "<br/>",
                                       paste0("<b>",pt2()$Station,"</b>"),
                                       paste("ID:",pt2()$ID),
                                       paste("Höhe:",pt2()$Höhe,"m"),
                                       paste("Messpr.:",pt2()$Messprodukte)),
